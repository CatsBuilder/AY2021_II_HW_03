...
volatile uint32_t accumulator_tmp=0;                                                //accumulators used in the average computation, it stores all the values sampled in a single sum
...
uint8_t sample=0;                                                                   //flag used to signal that sampling needs to be performed
...
int main(void){
    ...
    timer_Start();                                                                  //start the components and the isr -start
    AMux_Start(); 
    ADC_DelSig_Start();
    timer_isr_StartEx(CustomTimerISR);                                              //start the components and the isr -stop
    for(;;)
    {
        if(buffer_slave[1]!=ControlRegister2 || buffer_slave[0]!=ControlRegister1){ // if the value in at least one of the buffer's cointrol register is different from the stored copy of the control registers (the user has changed the control registers), we re-set the parameters and start again
            set_parameters();
            ...
            if(sample){                                                             //if a sample has to be taken, do it 
                ...
                data++;                                                             //we add 1 to the data counter
            }
